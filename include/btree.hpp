...
struct btree_iterator
{
    ...
    {
        return path.empty(); // Indicates if the iterator points to the end of the B-tree (not a valid entry)
    }
};
...
public:
    ...
    static btree_iterator end(far_offset_ptr btree_offset);
...
public:
    ...
    btree_iterator upsert(std::span<uint8_t> entry); // insert or update an entry in the B-tree
    std::vector<uint8_t> get_entry(btree_iterator it); // get the entry at the current iterator position
    btree_iterator insert(btree_iterator it, std::span<uint8_t> entry); // insert an entry at the current iterator position
    btree_iterator update(btree_iterator it, std::span<uint8_t> entry); // update the entry at the current iterator position
